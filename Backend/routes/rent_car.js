...
//user rental check out page
router.get("/rent/:id", async function (req, res, next) {
  try {

    const [rows, fields] = await pool.query("SELECT * FROM rental join car using(car_id) WHERE u_id = ? and r_status = 'รอชำระเงิน'", [req.params.id])
    return res.json(rows)
